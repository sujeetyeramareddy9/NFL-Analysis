...
def main():
    ...
    if pytorch:
        ...
        if predict == 'W-L':
            ...
            teams = get_team_stat_df()
            train_schedule, test_schedule = get_schedule(years)
            inferential_model, relationship_model = get_model_for_over_under(train_schedule, test_schedule, teams, 9)
            X_val, y_val, validation = predict_over_under(test_schedule, teams, 9, inferential_model)
